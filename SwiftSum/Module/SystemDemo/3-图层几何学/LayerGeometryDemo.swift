...
class LayerGeometryDemo: UIViewController {
    ...
    func tick() {
        ...
        let calender = NSCalendar(calendarIdentifier: NSCalendar.Identifier.buddhist)!
        ...
        miniteHand.transform = CGAffineTransform(rotationAngle: minsAngle)
        secondHande.transform = CGAffineTransform(rotationAngle:secseAngle)
    }
    ...
    func geometryFlippedDemo() {
        geometryFlippedView.layer.isGeometryFlipped = true
        geometryFlippedView2.layer.isGeometryFlipped = true
        geometryFlippedView2Label.layer.isGeometryFlipped = true
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let layerPoint = geometryFlippedView.layer.convert(point, from: view.layer)
        //get layer using containsPoint:
        if geometryFlippedView.layer.contains(layerPoint) {
            title = "Inside geometryFlippedView Layer"
        } else {
            title = "Outside geometryFlippedView Layer"
        }
