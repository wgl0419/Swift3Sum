...
class TransformDemo: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        addRightBarButtonItem(title: "仿射变换", action: #selector(affineTransform))
        threeDDemo()
    ...
    func affineTransform() {
        ...
        layer.frame = CGRect(x: 10, y: 200, width: 200, height: 200)
        layer.zPosition = 100
        ...
        YYGCD.dispatchInMainQueue(delay: 1000) {
    ...
    func threeDDemo() {
        ...
        /*
         但只是这样设置的话，结果看起来图层并没有被旋转，而是仅仅在水平方向上的一个压缩，是哪里出了问题呢？
         
         其实完全没错，视图看起来更窄实际上是因为我们在用一个斜向的视角看它，而不是透视。
         */
        
        /**
         透视投影 在真实世界中，当物体远离我们的时候，由于视角的原因看起来会变小
    ...
    /**
     ...
     这是一个BOOL类型，默认为YES，如果设置为NO，那么当图层正面从相机视角消失的时候，它将不会被绘制。结果是完全没有东西
     */
    func doubleSidedDemo() {
        ...
        imageViewDoubSided2.layer.isDoubleSided = false
    }
